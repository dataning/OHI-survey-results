...
jpeg(paste('figures/f1_classweights.jpg'),height=7,width=single,unit="in",res=res,qual=qual)
...
mp <- barplot(hh,las=3,ylim=c(0,1.2*max(hh+se)))
segments(mp, hh, mp, hh + se)
segments(mp+0.2, hh + se, mp-0.2, hh + se)
title(ylab='Relative Importance')
...
mp <- barplot(hh,
        ...
        args.legend = list(x=13,y=2.3,bty='n'))
title(ylab='Relative Importance')
segments(mp, hh, mp, hh + se)
segments(mp+0.2, hh + se, mp-0.2, hh + se)
dev.off()
